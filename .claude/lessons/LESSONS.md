...
## 2026-07-17 · python · Logger sem handler no driver do Glue engole o erro real (e enganou até a lição anterior)
- Sintoma: refs da silver falhavam e NENHUM `dataset_failed` aparecia no CloudWatch — nem pra IA de RCA da AWS; a lição da raw vazia culpou "flush do continuous logging".
- Causa raiz: `logging.getLogger('glue_silver').info(...)` sem handler configurado — o `lastResort` do logging só emite WARNING+; INFO é DESCARTADO, não atrasado. O grupo `/error` só recebe o log4j da JVM; o stdout do driver vai ao `/output` no fim do job.
- Regra: logger de runtime Glue nasce com StreamHandler explícito + nível INFO, e falha de item de pipeline loga em ERROR (`registrar_erro`), nunca INFO. Para depurar sem deploy: `StartJobRun` aceita override de `--extra-py-files` — zip patchado em chave S3 à parte + `--datasets` reduzido = erro real em 2 min.

## 2026-07-17 · python · Fonte não-determinística + MERGE UPDATE (Iceberg COW) = INVALID_NON_DETERMINISTIC_EXPRESSIONS
- Sintoma: só os 2 datasets ref-only (`geolocation`, `product_category_name_translation`) falhavam no job silver cold, na ANÁLISE do MERGE (~100ms após carregar o schema, zero Spark jobs); hubs/links/sats com os MESMOS dados passavam.
- Causa raiz: o MERGE copy-on-write com `WHEN MATCHED UPDATE` (plano `ReplaceData`) embute o plano INTEIRO da fonte num filtro de reescrita, e a análise (ANTES do prune de colunas) rejeita qualquer expressão não-determinística na árvore: `dropDuplicates` (aggregate com `first()`) E `input_file_name()` da leitura da raw — mesmo com a coluna `raw_file` já descartada no select. Trocar só o dedup por `row_number` não bastou: o `input_file_name()` lá embaixo ainda reprovava. MERGE insert-only (hubs/links) não gera `ReplaceData` — por isso só as refs quebravam.
- Regra: fonte de MERGE com UPDATE/DELETE precisa de plano-folha: `localCheckpoint()` no stage antes do MERGE (materializa e vira LogicalRDD determinístico); dedup determinístico (`linhas_unicas_por_chave`) continua valendo pela semântica. Falha na análise sem task alguma = ler o PLANO no erro (a expressão rejeitada aponta o nó), e validar o fix com o MESMO caminho que falhou antes de declarar resolvido.
